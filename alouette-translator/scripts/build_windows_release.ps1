...
# Check if exe exists
$exePath = "build\windows\x64\runner\Release\alouette-translator.exe"
if (-not (Test-Path $exePath)) {
    Write-Host "ERROR: Could not find alouette-translator.exe in Release folder" -ForegroundColor Red
...
if (Test-Path "$windowsOutputDir\alouette-translator.exe") {
    Write-Host "SUCCESS: alouette-translator.exe found in output directory" -ForegroundColor Green
} else {
    Write-Host "ERROR: alouette-translator.exe not found in output directory" -ForegroundColor Red
    Read-Host "Press Enter to exit"
    exit 1
}
...
try {
    $zipPath = "$outputDir\alouette-translator-windows-$version.zip"
    ...
    try {
        ...
        & 7z a "alouette-translator-windows-$version.zip" "windows\" | Out-Null
...
Files included:
- windows/alouette-translator.exe (Main executable)
- windows/data/ (Flutter assets and resources)
- windows/*.dll (Required libraries)
"@

if ($zipCreated) {
    $infoContent += "`n- alouette-translator-windows-$version.zip (Compressed package)"
}
...
Write-Host ""
Write-Host "Windows executable: $outputDir\windows\alouette-translator.exe" -ForegroundColor Green

if ($zipCreated) {
    Write-Host "ZIP package: $outputDir\alouette-translator-windows-$version.zip" -ForegroundColor Green
}
