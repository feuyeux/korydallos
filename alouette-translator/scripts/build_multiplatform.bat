...
if "%BUILD_ANDROID_APK%"=="true" (
    ...
    if not errorlevel 1 (
        ...
        if errorlevel 1 (
            copy "build\app\outputs\flutter-apk\app-release.apk" "!RELEASE_DIR!\android\alouette-translator-!VERSION_NAME!.apk" >nul 2>&1
        )
...
if "%BUILD_ANDROID_AAB%"=="true" (
    ...
    if not errorlevel 1 (
        ...
        REM 复制AAB文件到输出目录
        if not exist "!RELEASE_DIR!\android" mkdir "!RELEASE_DIR!\android"
        copy "build\app\outputs\bundle\release\app-release.aab" "!RELEASE_DIR!\android\alouette-translator-!VERSION_NAME!.aab"
        
        REM 生成文件信息
        echo   - alouette-translator-!VERSION_NAME!.aab >> "!RELEASE_DIR!\build_info.txt"
...
if "%BUILD_WINDOWS%"=="true" (
    ...
    if not errorlevel 1 (
        ...
        REM 检查源文件是否存在
        if exist "build\windows\x64\runner\Release\alouette-translator.exe" (
        ...
        where 7z >nul 2>&1
        if not errorlevel 1 (
            7z a "alouette-translator-windows-!VERSION_NAME!.zip" windows\ >nul
            echo 已创建ZIP压缩包
        ) else (
            where winrar >nul 2>&1
            if not errorlevel 1 (
                winrar a "alouette-translator-windows-!VERSION_NAME!.zip" windows\ >nul
                echo 已创建ZIP压缩包
            ) else (
        ...
        if exist "!RELEASE_DIR!\alouette-translator-windows-!VERSION_NAME!.zip" (
            echo   - alouette-translator-windows-!VERSION_NAME!.zip >> "!RELEASE_DIR!\build_info.txt"
        )
...
if "%BUILD_WEB%"=="true" (
    ...
    if not errorlevel 1 (
        ...
        where 7z >nul 2>&1
        if not errorlevel 1 (
            7z a "alouette-translator-web-!VERSION_NAME!.zip" web\ >nul
        ) else (
            where winrar >nul 2>&1
            if not errorlevel 1 (
                winrar a "alouette-translator-web-!VERSION_NAME!.zip" web\ >nul
            )
        )
        cd ..
        ...
        if exist "!RELEASE_DIR!\alouette-translator-web-!VERSION_NAME!.zip" (
            echo   - alouette-translator-web-!VERSION_NAME!.zip >> "!RELEASE_DIR!\build_info.txt"
        )
...
echo.
echo ### Windows
echo - 解压zip文件并运行alouette_translator.exe
