  ...
  ValidationResult validate(double value) {
    ...
    if (fieldName == 'defaultRate') {
      if (value < 0.5) {
        ...
        suggestions.add(
          'Consider using a rate between 0.8 and 1.2 for optimal listening',
        );
      } else if (value > 2.0) {
  ...
  ValidationResult validate(String value) {
    ...
    if (fieldName == 'outputDirectory') {
      if (value.contains(' ')) {
        ...
        suggestions.add(
          'Consider using paths without spaces for better compatibility',
        );
      }
    }
...
class VoiceValidationRule extends ValidationRule<String> {
  ...
  ValidationResult validate(String voiceName) {
    ...
    if (availableVoices == null || availableVoices!.isEmpty) {
      final warnings = [
        'Cannot verify voice availability - no voice list provided',
      ];
    ...
    if (voice == null) {
      final availableNames = availableVoices!
          .take(5)
          .map((v) => v.id)
          .join(', ');
      return ValidationResult.invalid([
    ...
    if (voice.isNeural) {
      ...
      if (neuralAlternatives.isNotEmpty) {
        suggestions.add(
          'Consider using neural voices for better quality: ${neuralAlternatives.join(', ')}',
        );
      }
    }
...
class TTSConfigValidator {
  ...
  ValidationResult _validateFeatureFlags(TTSConfig config) {
    ...
    if (!config.enableCaching && !config.enablePlayback) {
      ...
      suggestions.add(
        'Consider enabling at least one feature for better user experience',
      );
    }

    if (config.enableCaching) {
      suggestions.add(
        'Caching is enabled - this will improve performance for repeated text',
      );
    }
  ...
  ValidationResult _validateConsistency(TTSConfig config) {
    ...
    if (availableVoices != null) {
      final voice = availableVoices!
          .where((v) => v.id == config.defaultVoice)
          .firstOrNull;
      if (voice != null) {
        // Web平台建议
        if (voice.languageCode.startsWith('ar-') &&
            config.defaultFormat != 'mp3') {
          warnings.add(
            'Arabic voices may have better compatibility with MP3 format',
          );
        }
        ...
        if (voice.isNeural && config.defaultRate > 1.5) {
          suggestions.add(
            'Neural voices may work better with moderate speech rates',
          );
        }
      }
    }
    ...
    if (config.enableCaching && config.defaultFormat == 'wav') {
      warnings.add(
        'WAV format with caching enabled may consume significant storage',
      );
      suggestions.add(
        'Consider using MP3 format with caching for better storage efficiency',
      );
    }
  ...
  List<String> generateRecommendations(TTSConfig config) {
    ...
    if (config.defaultRate == 1.0 && config.defaultPitch == 1.0) {
      recommendations.add(
        'Default speech settings detected - consider personalizing rate and pitch',
      );
    }

    if (config.outputDirectory == 'output') {
      recommendations.add(
        'Using default output directory - consider specifying a custom path',
      );
    }

    if (!config.enableCaching) {
      recommendations.add(
        'Caching is disabled - enabling it can improve performance for repeated text',
      );
    }
      ...
      if (voice != null) {
        if (!voice.isNeural) {
          ...
          if (neuralAlternatives > 0) {
            recommendations.add(
              '$neuralAlternatives neural voice(s) available for ${voice.languageCode} - consider upgrading for better quality',
            );
          }
        }
      }
