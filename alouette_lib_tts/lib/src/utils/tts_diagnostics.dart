...
class TTSDiagnostics {
  ...
  static String generateReadableReport(Map<String, dynamic> diagnostics) {
    ...
    buffer.writeln('  预期可用性: ${flutterTTS['expectedAvailable'] ? '✓ 预期可用' : '✗ 预期不可用'}');
    buffer.writeln();
  ...
  static Map<String, dynamic> _getPlatformDiagnostics() {
    return {
      ...
      'platformType': PlatformUtils.isDesktop ? 'Desktop' : 
                      PlatformUtils.isMobile ? 'Mobile' : 
                      PlatformUtils.isWeb ? 'Web' : 'Unknown',
  ...
  static List<String> _generateRecommendations(Map<String, dynamic> diagnostics) {
    ...
    // Edge TTS 相关建议
    if (platform['platformType'] == 'Desktop' && !(edgeTTS['available'] as bool)) {
