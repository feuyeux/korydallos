...
/// Unified TTS Service - Centralized service for all TTS functionality
/// Provides consistent API across all applications with automatic platform detection
/// and engine switching capabilities
class UnifiedTTSService implements TTSServiceInterface {
  ...
  Future<void> initialize({
    ...
    try {
      ...
      _initialized = true;
      TTSLogger.initialization('Unified TTS service', 'completed', 'Using ${_currentEngine?.name} engine on ${_platformDetector.platformName}');
    } catch (e) {
      throw ErrorHandler.handleInitializationError(e, 'Unified TTS service');
    }
  }
  ...
  Future<void> switchEngine(
    ...
    try {
      ...
      // Apply current configuration to new processor
      await _applyConfiguration();
  ...
  Future<void> speakText(
    ...
    try {
      ...
      final audioData = await synthesizeText(text, selectedVoice, format: format);
      
      // Check if this is minimal audio data (direct playback mode)
      if (audioData.length <= 10) {
        TTSLogger.debug('Using direct playback mode - no additional audio processing needed');
  ...
  Future<void> stop() async {
    ...
    try {
      ...
      // Note: AudioPlayer might not have a stop method, this is a placeholder
    } catch (e) {
      throw TTSError(
  ...
  Future<void> setSpeechRate(double rate) async {
    ...
    _config = _config.copyWith(speechRate: rate);
  }
  ...
  void dispose() {
    TTSLogger.debug('Disposing Unified TTS service');
    try {
      _processor?.dispose();
      _audioPlayer = null;
    } catch (e) {
      ...
      _currentEngine = null;
      _initialized = false;
      TTSLogger.debug('Unified TTS service disposed successfully');
    }
  }

  /// Apply current configuration to the processor
  Future<void> _applyConfiguration() async {
    if (_processor == null) return;

    try {
      await _processor!.setSpeechRate(_config.speechRate);
      await _processor!.setPitch(_config.pitch);
      await _processor!.setVolume(_config.volume);
    } catch (e) {
      TTSLogger.warning('Failed to apply configuration: $e');
    }
  }
  ...
  void _ensureInitialized() {
    if (!isInitialized) {
      throw TTSError(
        'Unified TTS service is not initialized. Please call initialize() first.',
