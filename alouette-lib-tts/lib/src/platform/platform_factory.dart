...
class EdgeTTSAdapter extends PlatformAdapter {
  ...
  Future<bool> checkAvailability() async {
    ...
    try {
      ...
      TTSLogger.debug('Edge TTS availability check: $available');
      return available;
    } catch (e) {
...
class PlatformTTSFactory {
  ...
  static PlatformTTSFactory get instance => _instance ??= PlatformTTSFactory._();
  ...
  Future<PlatformAdapter?> selectBestAdapter() async {
    ...
    for (final adapter in recommended) {
      try {
        ...
        TTSLogger.warning('Failed to check availability for ${adapter.name}: $e');
        continue;
      }
    }
    ...
    TTSLogger.warning('No available adapters found, using fallback: ${fallback.name}');
    return fallback;
  }
