...
build_android_apk() {
    ...
    if [ $? -eq 0 ]; then
        ...
        cp build/app/outputs/flutter-apk/app-*.apk "${RELEASE_DIR}/android/" 2>/dev/null || {
            cp build/app/outputs/flutter-apk/app-release.apk "${RELEASE_DIR}/android/alouette-tts-${VERSION_NAME}.apk" 2>/dev/null
        }
...
build_android_aab() {
    ...
    if [ $? -eq 0 ]; then
        ...
        # 复制AAB文件到输出目录
        mkdir -p "${RELEASE_DIR}/android"
        cp build/app/outputs/bundle/release/app-release.aab "${RELEASE_DIR}/android/alouette-tts-${VERSION_NAME}.aab"
        
        # 生成文件信息
        size=$(ls -lh "${RELEASE_DIR}/android/alouette-tts-${VERSION_NAME}.aab" | awk '{print $5}')
        echo "  - alouette-tts-${VERSION_NAME}.aab (${size})" >> "${RELEASE_DIR}/build_info.txt"
...
build_ios() {
    ...
    if [ $? -eq 0 ]; then
        ...
        cp build/ios/ipa/*.ipa "${RELEASE_DIR}/ios/alouette-tts-${VERSION_NAME}.ipa" 2>/dev/null || {
            echo -e "${YELLOW}未找到IPA文件，可能需要配置签名证书${NC}"
            # 复制Runner.app
            if [ -d "build/ios/iphoneos/Runner.app" ]; then
                cp -r "build/ios/iphoneos/Runner.app" "${RELEASE_DIR}/ios/alouette-tts-${VERSION_NAME}.app"
...
build_macos() {
    ...
    if [ $? -eq 0 ]; then
        ...
        cp -r build/macos/Build/Products/Release/alouette_tts.app "${RELEASE_DIR}/macos/Alouette-TTS-${VERSION_NAME}.app"
...
build_windows() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if command -v zip &> /dev/null; then
            cd "${RELEASE_DIR}"
            zip -r "alouette-tts-windows-${VERSION_NAME}.zip" windows/
            cd - > /dev/null
        fi
        ...
        if [ -f "${RELEASE_DIR}/alouette-tts-windows-${VERSION_NAME}.zip" ]; then
            zip_size=$(ls -lh "${RELEASE_DIR}/alouette-tts-windows-${VERSION_NAME}.zip" | awk '{print $5}')
            echo "  - alouette-tts-windows-${VERSION_NAME}.zip (${zip_size})" >> "${RELEASE_DIR}/build_info.txt"
        fi
...
build_linux() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if command -v tar &> /dev/null; then
            cd "${RELEASE_DIR}"
            tar -czf "alouette-tts-linux-${VERSION_NAME}.tar.gz" linux/
            cd - > /dev/null
        fi
        ...
        if [ -f "${RELEASE_DIR}/alouette-tts-linux-${VERSION_NAME}.tar.gz" ]; then
            tar_size=$(ls -lh "${RELEASE_DIR}/alouette-tts-linux-${VERSION_NAME}.tar.gz" | awk '{print $5}')
            echo "  - alouette-tts-linux-${VERSION_NAME}.tar.gz (${tar_size})" >> "${RELEASE_DIR}/build_info.txt"
        fi
...
build_web() {
    ...
    if [ $? -eq 0 ]; then
        ...
        if command -v zip &> /dev/null; then
            cd "${RELEASE_DIR}"
            zip -r "alouette-tts-web-${VERSION_NAME}.zip" web/
            cd - > /dev/null
        fi
        ...
        if [ -f "${RELEASE_DIR}/alouette-tts-web-${VERSION_NAME}.zip" ]; then
            zip_size=$(ls -lh "${RELEASE_DIR}/alouette-tts-web-${VERSION_NAME}.zip" | awk '{print $5}')
            echo "  - alouette-tts-web-${VERSION_NAME}.zip (${zip_size})" >> "${RELEASE_DIR}/build_info.txt"
        fi
...
### Windows
- 解压zip文件并运行alouette_tts.exe

### Linux
- 解压tar.gz文件并运行alouette_tts
